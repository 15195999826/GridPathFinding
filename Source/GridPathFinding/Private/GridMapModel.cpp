		...
		case EGridMapType::RECTANGLE_SIX_DIRECTION:
			{
				...
				FVector v{ (MapConfig.TileOrientation == ETileOrientationFlag::FLAT) ? 
					FVector(q, -q - r, r) : FVector(q, r, -q - r) };
...
FIntVector2 UGridMapModel::StableCoordToRowColumn(const FHCubeCoord& InCoord) const
{
	switch (MapConfig.MapType)
	{
		case EGridMapType::HEX_STANDARD:
...
FHCubeCoord UGridMapModel::StableRowColumnToCoord(const FIntVector2& InRowColumn) const
{
	switch (MapConfig.MapType)
	{
		case EGridMapType::HEX_STANDARD:
